...
public class BodyPartsManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Debug.Log("Animator Runtime Controller: " + animator.runtimeAnimatorController);

        animatorOverrideController = new AnimatorOverrideController(animator.runtimeAnimatorController);
        animator.runtimeAnimatorController = animatorOverrideController;
        Debug.Log("Animator Override Controller: " + animatorOverrideController);
        ...
        Debug.Log("Initial Animation Overrides:");
        foreach (var clipOverride in defaultAnimationClips)
        {
            Debug.Log("Override Key: " + clipOverride.Key.name + ", Value: " + (clipOverride.Value != null ? clipOverride.Value.name : "null"));
        }
    ...
    public void UpdateBodyParts()
    {
        ...
        for (int partIndex = 0; partIndex < bodyPartTypes.Length; partIndex++)
        {
            ...
            for (int stateIndex = 0; stateIndex < characterStates.Length; stateIndex++)
            {
                ...
                for (int directionIndex = 0; directionIndex < characterDirections.Length; directionIndex++)
                {
                    ...
                    {
                        Debug.Log("Loaded Animation Clip: " + animationClip.name);
                    }
                    else
                    {
                        Debug.LogWarning("Failed to load Animation Clip: " + "Player Animations/" + partType + "/" + partType + "_" + partID + "_" + state + "_" + direction);
                    }
        ...
        Debug.Log("Updated Animation Overrides:");
        foreach (var clipOverride in defaultAnimationClips)
        {
            Debug.Log("Override Key: " + clipOverride.Key.name + ", Value: " + (clipOverride.Value != null ? clipOverride.Value.name : "null"));
        }
